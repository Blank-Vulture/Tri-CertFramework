...
:::caution[過去バージョン]
これは過去バージョン（v2.46）です。最新版は [v2.53](./thesis-v2-53/) をご覧ください。
:::
...
zk-SNARKsは、前節で述べたZKPの課題（証明者と検証者が同時に通信する必要がある）を解決した方式である。証明者が一度だけ証明データを作成すれば、検証者はいつでも・何度でも検証できる。双方が同時に通信する必要がなくなったことで、ZKPを実際のシステムに組み込みやすくなった。

　本研究が目指す「PDFに証明を埋め込んで、いつでも誰でも検証できる」というユースケースでは、この性質が不可欠である。
