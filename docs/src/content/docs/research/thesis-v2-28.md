...
:::caution[過去バージョン]
これは過去バージョン（v2.28）です。最新版は [v2.44](./thesis-v2-44/) をご覧ください。
:::
...
| 用語 | 正式名称 | 本研究での役割 |
|------|----------|----------------|
| ZKP | Zero-Knowledge Proof（ゼロ知識証明） | 証明書の内容を明かさずに「本物である」ことだけを証明する |
| zk-SNARKs | Zero-Knowledge Succinct Non-Interactive Argument of Knowledge | ZKPの実装方式。証明データが小さく高速に検証できる |
| Groth16 | （プロトコル名） | 本システムで採用したzk-SNARKsの具体的な方式 |
| Poseidon | （ハッシュ関数名） | ZKP回路内で効率的に動作するハッシュ関数 |
| WebAuthn | Web Authentication API | ブラウザ上でパスキー認証を実現するW3C標準 |
| FIDO2 | Fast IDentity Online 2 | パスワードレス認証の業界標準規格 |
| CTAP | Client to Authenticator Protocol | PCとセキュリティキー間の通信規格 |
| VK | Verification Key（検証鍵） | ZKP証明を検証するための公開鍵 |
| PKI | Public Key Infrastructure | 従来のデジタル証明書の基盤技術 |
| X.509 | （規格名） | 従来のデジタル証明書の標準形式 |
