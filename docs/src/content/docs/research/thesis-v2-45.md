...
:::caution[過去バージョン]
これは過去バージョン（v2.45）です。最新版は [v2.52](./thesis-v2-52/) をご覧ください。
:::
...
　ここで「暗号学的手法」とは、数学的な計算の困難さを利用してデータの安全性を保証する技術の総称である。例えば、「ある計算は簡単にできるが、その逆の計算は現在のコンピュータでも何百年もかかる」という数学的性質を利用することで、データの改竄や偽造を検出可能にする。本研究では主に以下の3つの技術を組み合わせて使用する。
