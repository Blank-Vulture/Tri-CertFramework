...
:::caution[過去バージョン]
これは過去バージョン（v2.43）です。最新版は [v2.44](./thesis-v2-44/) をご覧ください。
:::
...
本研究では、上述の3つの課題を解決するため、暗号学的手法を用いた新しい文書真正性検証フレームワーク **Tri-CertFramework** を提案・実装する。本フレームワークで使用する具体的な技術については第2章で詳述するが、本節ではその目的と解決策の概要を述べる。
...
<p align="center"><strong>表1.2: 本研究の定量的目標</strong></p>
...
| 技術カテゴリ | 本研究での役割 | 詳細 |
|-------------|---------------|------|
| 暗号学的証明技術 | 内容を開示せずに真正性を証明 | 2.2節 |
| 生体認証技術 | 証明書所有者の本人確認 | 2.3節 |
| ハッシュ関数 | 改竄検知と高速な証明生成 | 2.2.3節 |
...
zk-SNARKsは、ZKPを実用的に使えるようにした技術である。「Succinct（簡潔）」は証明データが小さいこと、「Non-Interactive（非対話型）」は証明者と検証者が何度もやり取りする必要がないことを意味する。従来のZKPでは、証明者と検証者が何度も情報をやり取りする必要があったが、zk-SNARKsでは証明者が一度だけ証明データを生成すれば、検証者はそれを受け取るだけで検証を完了できる。
...
身近な例で説明すると、銀行口座を開設する際に本人確認書類を提出し、銀行が「この人は確かに存在する正当な顧客である」と認めて口座番号を発行するのと同様である。本システムでは、大学が「この学生は確かに本学を卒業した」と認めて登録を行う。
...
<p align="center"><strong>表3.3: 各認証層が防ぐ攻撃</strong></p>
...
図3.28にハッシュ計算アルゴリズムの実装を示す。学生の認証に使用するハッシュは、Salt、正規化された氏名・生年月日、および学籍番号から生成される。ここでの狙いは、個人情報そのものを公開せずに「登録済みである」という事実だけを証明できる形に変換することである。
...
登録情報は`commit-allowlist.json`として公開される。図3.29にAllowlistファイルの構造を示す。このファイルには個人情報は含まれず、ハッシュ値のみが記録される。検証者にとっては「発行機関が確かに登録した」という公開証拠として機能する。
...
図3.31にマニフェストファイルの構造を示す。マニフェストファイルには、ファイルの整合性検証に必要なハッシュ値とLedger署名情報が含まれる。検証者はこの情報を使って「検証鍵が改竄されていない」ことを確認できる。
...
図3.32に証明生成フローの実装を示す。証明生成は5つのステップで構成される。Step1は発行機関の登録済み確認、Step2は改竄検知、Step3は選択的開示を可能にするZKP生成、Step4は本人性確認、Step5は証明情報の証明書内包（照会不要化）を担当する。
...
本システムで使用するZKP回路は、Poseidonハッシュ[11]を用いたコミットメントスキームに基づいている。ここで示したい要点は「証明書の内容は隠しつつ、正しい内容であることだけを証明する」点である。図3.33に回路の概念的な構造を示す。
...
生成された証明は、PDFのメタデータ（Subject フィールド）にBase64エンコードして埋め込まれる。暗号化されたPDFの場合は、ファイル末尾に追記する方式（tail-append）を使用する。これにより、検証に必要な情報が証明書自身に内包され、発行機関への照会を不要にできる。図3.34に証明バンドルのインターフェース定義を示す。
...
この5ステップは、(1)文書が改竄されていないこと、(2)証明が正しいこと、(3)本人性と登録状況が一致することを順に確認する流れである。
...
Groth16[8]証明の検証は、snarkjs[9]ライブラリを使用して行われる。ここで確認しているのは「証明書の中身を開示せずに、改竄されていないことを示せるか」である。図3.35にZKP検証の実装を示す。
...
WebAuthn[12]署名の検証は、クライアントサイドで完結する。ここでは「証明書を提示している人物が本人であるか」を確認する。図3.36にWebAuthn署名検証の実装を示す。
...
本システムで使用する主要なデータスキーマを図3.37、図3.38に示す。スキーマを明示することで、誰が検証しても同じ手順で確認できる状態を確保する。
...
<p align="center"><strong>表4.1b: 本システムの処理時間内訳</strong></p>
...
本節の要点は、現実的な計算時間では偽造が成立しないという点にある。
...
プライバシー上の重要な考慮事項として、リンカビリティ（紐付け可能性）の分析を行った。ここでいうリンカビリティとは、別々の証明書から「同一人物だ」と推測できてしまうリスクを指す。
...
本研究で示した技術的実現性を踏まえ、次段階で取り組むべき課題を整理する。
