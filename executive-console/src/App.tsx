  ...
  const testListDevices = async () => {
    ...
    try {
      ...
      result.forEach((dev, i) => {
        addLog(`  Device ${i + 1}: ${dev.product_string} (${dev.manufacturer_string})`)
  ...
  const testGetPublicKey = async () => {
    ...
    try {
      ...
      if (errorStr.includes('App not running') || errorStr.includes('6d02')) {
        ...
        addLog('3. 画面に "Application is ready" が表示されることを確認', 'info')
    ...
    <div className="space-y-4">
      ...
      {devices.length > 0 && (
        <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
          <h4 className="text-sm font-medium text-green-800 dark:text-green-200 mb-2">検出されたデバイス</h4>
          {devices.map((dev, i) => (
            <div key={i} className="text-xs text-green-700 dark:text-green-300 font-mono">
      ...
      {publicKey && (
        <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
          <h4 className="text-sm font-medium text-purple-800 dark:text-purple-200 mb-2">公開鍵情報</h4>
      ...
      {signature && (
        <div className="p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
          <h4 className="text-sm font-medium text-amber-800 dark:text-amber-200 mb-2">署名結果</h4>
      ...
      {logs.length > 0 && (
        <div className="rounded-lg border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 p-3 max-h-64 overflow-auto">
          <div className="space-y-1">
            {logs.map((log, index) => (
              <div key={index} className="text-xs font-mono text-gray-700 dark:text-slate-300">
                {log}
              </div>
            ))}
          </div>
