...
:::caution[過去バージョン]
これは過去バージョン（v1.4）です。最新版は [v2.52](./thesis-v2-52/) をご覧ください。
:::
...
図2.X に示すように、本研究は「分散型」かつ「プライバシー保護」の両方を高い水準で実現している。従来のX.509/PKIは認証局への依存度が高く、Open Badgesは発行者サーバーへの依存がある。Blockcertsやハイパーレジャー系のSSIはブロックチェーンにより分散性を確保しているが、プライバシー保護は限定的である。
...
図3.X にRegistrar Consoleのメイン画面を示す。
...
図3.X にExecutive Consoleのメイン画面を示す。
...
VKバンドルの署名には、ハードウェアウォレットLedgerを使用する。図3.X にLedger署名プロセスを示す。
