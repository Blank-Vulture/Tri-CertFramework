...
:::caution[過去バージョン]
これは過去バージョン（v2.29）です。最新版は [v2.45](./thesis-v2-45/) をご覧ください。
:::
...
| 用語 | 正式名称 | 本研究での役割 |
|------|----------|----------------|
| ZKP | Zero-Knowledge Proof（ゼロ知識証明） | 中身を見せずに「本物」と証明する技術 |
| zk-SNARKs | Zero-Knowledge Succinct Non-Interactive Argument of Knowledge | 小さく・速く・一方向で検証できるZKPの方式 |
| Groth16 | | 本システムで採用したzk-SNARKsの方式 |
| Poseidon | | ZKP向けに設計されたデータ変換関数 |
| WebAuthn | Web Authentication API | 指紋やUSBキーでログインするWeb標準 |
| FIDO2 | Fast IDentity Online 2 | パスワード不要の認証を実現する業界規格 |
| CTAP | Client to Authenticator Protocol | PCとUSBキー間の通信規格 |
| VK | Verification Key（検証鍵） | 証明が正しいか確認するための公開データ |
| PKI | Public Key Infrastructure | 従来の電子証明書を支える仕組み |
| X.509 | | 従来の電子証明書の形式 |
