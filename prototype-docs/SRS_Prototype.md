...
**版 2.4  最終更新: 2025-08-09（更新: Groth16統一 / Phase0 QR回避 / Hardhat推奨）**
...
## 2. スコープ
### 含まれる（段階的移行対応）
* **Phase 0（基盤）**: Scholar Prover PWA + Verifier UI SSG + **ローカルファイル指定によるVK取得機能** + **WebAuthn署名（JWS非採用）**
...
* **電子署名機能**: パスキー電子署名（WebAuthnアサーション）生成・検証、検証鍵リポジトリ管理（JWSは使用しない）
...
* Phase 0 における QR エクスポート（WebAuthnアサーション方式で不要）
* 多言語対応・アクセシビリティ
...
### 5.2 Phase 1: ブロックチェーン統合（ネットワーク方針更新）
...
#### ブロックチェーン統合
- **ネットワーク**: デフォルトは Hardhat/Hardhat Network（ローカル）。zkEVM Cardona はオプション。
