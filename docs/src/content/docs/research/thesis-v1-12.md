...
:::caution[過去バージョン]
これは過去バージョン（v1.12）です。最新版は [v2.27](./thesis-v2-27/) をご覧ください。
:::
...
システムは、利用者の登録を行うRegistrar Console、検証鍵を管理するExecutive Console、証明を生成するProver、検証を行うVerifierの4つのコンポーネントで構成される。卒業証明書を対象として実装し、医療事務従事者を含む協力者による評価を実施した。
