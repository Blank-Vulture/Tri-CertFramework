...
:::caution[過去バージョン]
これは過去バージョン（v2.50）です。最新版は [v2.60](./thesis-v2-60/) をご覧ください。
:::
...
　本フレームワークは、表1.1aに示す3層構造で証明書の真正性を保証する。

<p align="center"><strong>表1.1a: 3層認証構造</strong></p>
...
　図3.2aに、証明書の発行から検証までのデータフローを示す。本図では、プロセス（角丸矩形）とデータストア（円筒形）を明確に分離し、各矢印にはデータ名を記載している。
...
<p align="center"><strong>図3.2a: データフロー図</strong></p>
...
<p align="center"><strong>表4.1a: 5段階評価の定義</strong></p>
...
　これらの保証に基づき、表4.8に想定される攻撃シナリオと対策を示す。

### 4.3.3 攻撃シナリオ分析

<p align="center"><strong>表4.8: 攻撃シナリオと対策</strong></p>
...
　表4.8は、想定される不正を「どこで止めるか」という視点で整理したものである。残存リスクは運用上の対策で補完する必要がある。

## 4.4 既存手法との比較

### 4.4.1 比較対象

　本システムを以下の既存手法と比較し、その結果を表4.9に示す。
...
　表4.9は、検証者の実務上の利得（速さ・わかりやすさ・本人性確認）に焦点を当てて整理したものである。

<p align="center"><strong>表4.9: 既存手法との比較</strong></p>
