...
export default function VerificationResults({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Certificate Information - Year, Circuit ID, etc. */}
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-gray-900 text-lg mb-3">
                {t("results.certificateInfo.title")}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {/* Graduation Year */}
                <div className="bg-white/60 rounded-lg px-3 py-2">
                  <p className="text-xs text-gray-500">
                    {t("results.certificateInfo.graduationYear")}
                  </p>
                  <p className="text-lg font-bold text-blue-700">
                    {result.certificateInfo.graduationYear
                      ? `${result.certificateInfo.graduationYear}年`
                      : t("results.certificateInfo.unknownYear")}
                  </p>
                </div>
                {/* Hash Method */}
                <div className="bg-white/60 rounded-lg px-3 py-2">
                  <p className="text-xs text-gray-500">
                    {t("results.certificateInfo.hashMethod")}
                  </p>
                  <p className="text-sm font-medium text-gray-700">
                    {result.certificateInfo.hashMethod === "raw"
                      ? t("results.certificateInfo.hashMethodRaw")
                      : t("results.certificateInfo.hashMethodNormalized")}
                  </p>
                </div>
                {/* Circuit ID */}
                <div className="bg-white/60 rounded-lg px-3 py-2 sm:col-span-2">
                  <p className="text-xs text-gray-500">
                    {t("results.certificateInfo.circuitId")}
                  </p>
                  <p className="text-sm font-mono text-gray-700 break-all">
                    {result.certificateInfo.circuitId}
                  </p>
                </div>
                {/* VKey Hash */}
                <div className="bg-white/60 rounded-lg px-3 py-2 sm:col-span-2">
                  <p className="text-xs text-gray-500">
                    {t("results.certificateInfo.vkeyHash")}
                  </p>
                  <p className="text-xs font-mono text-gray-600 break-all">
                    {result.certificateInfo.vkeyHash}
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
        ...
        {overallValid ? (
          <div className="space-y-3 text-sm text-gray-700">
            <p>
              ✅ <strong>{t("results.meaning.authentic")}</strong> -{" "}
              {t("results.meaning.authenticDesc")}
            </p>
            <p>
              ✅ <strong>{t("results.meaning.unchanged")}</strong> -{" "}
              {t("results.meaning.unchangedDesc")}
            </p>
            <p>
              ✅ <strong>{t("results.meaning.verified")}</strong> -{" "}
              {t("results.meaning.verifiedDesc")}
            </p>
          </div>
        ) : (
          <div className="space-y-3 text-sm text-gray-700">
            ...
            <ul className="list-disc list-inside text-gray-600 space-y-1">
              ...
              {!result.signatureValid && (
                <li>{t("results.issue.signature")}</li>
              )}
              {!result.hashValid && <li>{t("results.issue.hash")}</li>}
              {!result.vkeyHashValid && <li>{t("results.issue.vkeyHash")}</li>}
              {result.saltRegistrationValid === false && (
                <li>{t("results.issue.saltRegistration")}</li>
              )}
            </ul>
          </div>
        )}
...
function ResultRow({
  ...
  return (
    <div
      className={`p-4 flex items-start gap-4 ${isImportant ? "bg-amber-50/50" : ""}`}
    >
