...
:::caution[過去バージョン]
これは過去バージョン（v2.38）です。最新版は [v2.60](./thesis-v2-60/) をご覧ください。
:::
...
1. **改竄耐性（多層認証）**: 暗号学的証明、生体認証による署名、発行機関の登録という複数層の認証により、偽造を暗号学的に実行不可能にする
2. **即時性（PDF埋め込み検証）**: 証明データをPDFに直接埋め込むことで、発行機関への照会なしに数秒で検証完了
3. **秘匿性（選択的開示）**: 検証に必要な最小限の情報のみを開示し、プライバシーを保護
...
Poseidonは、ZKP回路での使用を前提に設計されたハッシュ関数である。「有限体」と呼ばれる数学的構造上での演算のみを使用する。有限体とは、一定の範囲内の整数のみを扱い、足し算や掛け算の結果がその範囲を超えると「巻き戻る」性質を持つ数の体系である（時計の12時間制に似ている）。ZKP回路は本来この有限体上で動作するため、Poseidonはビット演算への変換が不要となり、制約数を大幅に削減できる。
